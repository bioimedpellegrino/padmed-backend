...
class RecordVideoView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            measurement_id, logs = asyncio.run(make_measure(config=config, config_path=config_path, video_path=video_path, demographics=anagrafica, start_time=0, end_time=30, chunks_duration=5))
            # Logger
